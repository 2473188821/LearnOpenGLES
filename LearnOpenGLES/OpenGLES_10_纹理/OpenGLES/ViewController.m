...
- (void)glkView:(GLKView *)view drawInRect:(CGRect)rect {
    ...
    if (canInvert) {
        GLuint modelMatrixUniformLocation = glGetUniformLocation(program, "normalMatrix");
        glUniformMatrix4fv(modelMatrixUniformLocation, 1, 0, normalMatrix.m);
    }

    GLuint lightDirectionUniformLocation = glGetUniformLocation(program, "lightDirection");
    glUniform3fv(lightDirectionUniformLocation, 1,self.lightDirection.v);
    
    // 绑定纹理
    GLuint diffuseMapUniformLocation = glGetUniformLocation(program, "diffuseMap");
